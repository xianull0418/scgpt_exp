...
parser.add_argument(
    "--fp16",
    action="store_true",
    help="Whether to train in automatic mixed precision with FP16. Default is False.",
)
parser.add_argument(
    "--bf16",
    action="store_true",
    help="Whether to train in automatic mixed precision with BF16 (for H100 GPUs). Default is False.",
)
parser.add_argument(
    "--tf32",
    action="store_true",
    help="Whether to enable TF32 precision for matrix multiplications (for A100/H100 GPUs). Default is False.",
)
parser.add_argument(
    "--fast-transformer",
    type=bool,
    default=True,
    help="Whether to use the fast transformer. Default is False.",
)
...
# 确保从环境变量中获取LOCAL_RANK
if 'LOCAL_RANK' in os.environ:
    args.local_rank = int(os.environ['LOCAL_RANK'])
    print(f"Setting local_rank from environment variable: {args.local_rank}")
else:
    print(f"WARNING: LOCAL_RANK not found in environment variables, using default: {args.local_rank}")
...
if IS_DATA_PARALLEL:
    ...
    # 使用local_rank对应的GPU，而不是总是使用cuda:0
    device = torch.device(f"cuda:{args.local_rank}")
    n_gpu = torch.cuda.device_count()
    world_size = torch.distributed.get_world_size()
    logger.info(
        f"device: {device} in world size {world_size}, "
        f"visible gpu(s): {os.environ.get('CUDA_VISIBLE_DEVICES', 'all')}/{n_gpu}"
    )
else:
...
# 为H100设置TF32精度
if args.tf32:
    # 对于A100和H100，启用TF32精度加速矩阵乘法(GEMM)和卷积操作
    torch.backends.cuda.matmul.allow_tf32 = True
    torch.backends.cudnn.allow_tf32 = True
    logger.info("启用TF32精度用于矩阵乘法和卷积")
...
# 初始化梯度缩放器
scaler = torch.cuda.amp.GradScaler(enabled=args.fp16)  # 注意:bf16不需要梯度缩放
...
def train(model: nn.Module, train_loader: DataLoader, epoch: int) -> None:
    ...
    for batch, data_dict in enumerate(train_loader):
        ...
        # 不再使用USE_GENERATIVE_TRAINING来区分数据处理方式
        # 统一使用标准数据格式
        ...
        total_gen += 0.0  # 这里原本是loss_gen，现在没有了
